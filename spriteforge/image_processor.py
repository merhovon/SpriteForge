...
class ImageProcessor:
    ...
    def save_sprite(
        self, selection: Tuple[int, int, int, int], name: str = "sprite"
    ) -> str:
        """
    ...
    def find_unique_colors(
        self, selection: Tuple[int, int, int, int], progress_callback=None
    ) -> List[List[int]]:
        """
    ...
    def create_unique_colors_image(
        self, unique_colors: List[List[int]]
    ) -> Optional[PILImage.Image]:
        """
        Create a 1D strip image from unique colors (n×1 image)

        Args:
            unique_colors: List of RGB colors

        Returns:
            PIL Image (n×1 pixels)
        """
    ...
    def save_unique_colors(
        self, unique_colors: List[List[int]], name: str = "unique"
    ) -> str:
        """
        ...
        Returns:
            Path to saved file
        """
        image = self.create_unique_colors_image(unique_colors)
        if image:
            output_path = self._get_output_path(name)
    ...
    def extract_transparent_sprite(
        self, selection: Tuple[int, int, int, int], progress_callback=None
    ) -> Optional[PILImage.Image]:
        """
        ...
        for idx, file_path in enumerate(png_files):
            try:
                # Create temporary processor for this image
                temp_proc = ImageProcessor(str(file_path))
                section = temp_proc.get_selection_image(selection)
                if section:
    ...
    def get_region_info(self, x: int, y: int) -> dict:
        ...
        try:
            ...
            return {"x": x, "y": y, "color": pixel}
        except Exception:
            return {"x": x, "y": y, "color": None}

    def crop_to_selection(self, selection: Tuple[int, int, int, int]):
        """
        Crop the loaded image to the selected region (DESTRUCTIVE).

        Args:
            selection: (x, y, width, height)

        Note:
            This modifies self.image in-place.
        """
        if not self.image:
        ...
        if width <= 0 or height <= 0:
            raise ValueError(f"Invalid selection size: {width}x{height}")

        # Validate bounds
        img_w, img_h = self.image.size
        if x < 0 or y < 0 or x + width > img_w or y + height > img_h:
            raise ValueError(
                f"Selection out of bounds: ({x},{y})+({width},{height}) > ({img_w},{img_h})"
            )
